...
3. **Owner decisions, none blocking code:**
   - **Movement's multiplicative gear x tree violation is CONFORMED — judge
     ...
     +20/+20: 1584 -> 1540 cm/s, -2.8%). Nothing was retuned; only the
     composition changed. Full before/after table in `Docs/Movement-Design.md`.
     If it feels wrong the fix is to retune the CONTENT (affix ranges, node
     percentages), not to put the multiplication back.
