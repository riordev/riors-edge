...
Source/RiorsEdge/
  ...
  Tests/          60 automation tests (RiorsEdge.* filter)
...
2. **UI follow-ups** (the style-guide implementation itself is done — see
   ...
   unimplemented.
3. Caster: the Spellblade abilities and Unmake are live (see the Caster
   bullet above). What remains is the `ClassResourceFloor` attribute (spec
   D8) — without it Overcast can never be entered, which silently disables
   the class's defining mechanic — then Rot/Siphon/Fracture/Resonance and
   the Combat/ systems they need (zones, partial healing, a projectile base,
   status consumption). Also: Overdrive keystone branch stubs and the
...
- Tests: headless `UnrealEditor-Cmd.exe` with `-ExecCmds="Automation RunTests RiorsEdge; Quit" -unattended -nop4 -nosplash -nullrhi`; 60 pass. NOTE: when the owner has the MAIN tree editor open, an agent building in a separate worktree hits a false-positive Live Coding lock (the guard keys off the shared UnrealEditor.exe, not the project DLL); `-NoHotReloadFromIDE` is the correct override in that case only.
