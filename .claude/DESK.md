...
- ACT I CHAIN AFTER THE QUARTERMASTER TURN-IN: owner reports the Kess salvage choice was ABSENT after the first contract is turned in. The chain is REACHABLE: the Act I+II loop probe drives "What would heat the Forge? -> Quest.KessSalvage.Offered" immediately after Quest.FirstContract.TurnedIn in a real world through real visible choices, and reaches Quest.Breach with four Doctrine. The probe reads GetVisibleChoices directly and says so — not a Slate input test. Remaining suspect is the PLATE: that node offers six choices, BuildFrame scrolls the body in an SScrollBox with no visible scrollbar, and the "1 - N CHOOSE" footer is inside the scrolling region, so an overflowing plate hides the later choices AND the count that would reveal them. Unconfirmed: no capture fixture seeds those flags, so the plate has never been photographed in that state. Next step is that fixture, not another data audit.
