...
## Cycle — Primary magazine and range affixes
- [ ] Add rollable Primary-only magazine capacity and effective range targets, with explicit O2 anchors on the current T12-to-T-1 ladder.
- [ ] Apply capacity before temporary Loaded rounds; conserve ammunition across gear changes, reload, slot swaps and capacity shrink.
- [ ] Apply range to real falloff and trace/projectile reach, or exclude archetypes without a meaningful consumer from the roll pool.
- [ ] Verify actual equipped items change those weapon behaviors without leaking to the Secondary; review, build, census and full suite before landing.
