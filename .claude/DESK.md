...
- **Quieter vacant gear slots:** Removed repeated EMPTY labels and centered the retained slot names beside icon wells; removed EMPTY from the backpack hint. Existing outlines and equip interaction geometry remain. Build passed;668 tests,665 passed,3known,0unexpected via quarantine auditor. Icon wells now use two-pixel outlines to retain all edges at reduced UI scale; vacant rows use minimum height instead of a hard cap. Four additional inventory frames inspected at720p/1080p with complete icon outlines. STATE unchanged under host-Python quarantine.
