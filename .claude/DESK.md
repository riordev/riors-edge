...
- CAST TIME, PARTIALLY LANDED (O266). The mechanism is class-wide and Cleave is the only ability authoring a wind-up (0.35s). REMAINING, and it is real work: Rot 0.60, Siphon 0.50, Fracture 0.70, Resonance 0.80 and Unmake 0.90 were authored, measured and REVERTED in this cycle because they turn 38 Caster runtime tests red at once — every one of them encodes "the effect lands on the press", which is exactly what O266 changes. Cleave alone cost 6. Budget the other five as their own block with the fixture work, not as a data edit. Also outstanding: the Cast Speed lane (BeginCastIfNeeded reads 1.0 deliberately; needs a canon row in power-and-scaling.md plus a conformance test before any node or affix bids into it), and cast times for the other four classes, which needs a per-ability instant/wind-up classification the owner has not given.
...
- CONSEQUENCE OF O266 worth the owner's hands: a parry window is 0.25s and Cleave's wind-up is 0.35s, so a parry pressed when the caster starts swinging has LAPSED by impact. The defender must now react to the wind-up rather than pre-empt it. That is arguably the point of a wind-up, but it is a real change to how parry feels and nobody has played it.
