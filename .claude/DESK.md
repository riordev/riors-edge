...
## Cycle — Remaining Primary utility and traversal affixes
- [ ] Add Primary sustained-fire accuracy that reduces accumulated bloom, preserving first-shot spread, recoil and movement penalties.
- [ ] Add discrete Primary hitscan pierce (+1 at T4 through T0, +2 at T-1) with legal roll/treatment rules and actual penetration tests; exclude rockets without a penetration consumer.
- [ ] Migrate the saved Offense.WallRideDamage identity to actual completed traversal, preserving existing item values and rejecting aborted movement.
- [ ] Verify real equipped items and actual weapon/traversal actions, review, build, census and full suite before landing.
Remaining repairs include Pierce/Sustained Fire Accuracy, silent nodes, ability balance, rarity identities, other weapon poses and audible combat polish. Rifle imported materials, fixed magazine/stock reload gesture and the native additive fire layer remain art/animation work; remote clients still lack a reload clock.
