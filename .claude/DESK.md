...
- ACT I CHAIN AFTER THE QUARTERMASTER TURN-IN: owner reports the chain breaks and does not start again after the first contract is turned in. NOT REPRODUCED from data. The shipped graph is intact — every Act1.Fernhall beat's completion flag has an available dialogue choice that sets it, the post-turn-in Quartermaster entry resolves to Done and reaches Start, Kess's Salvage offer requires exactly Quest.FirstContract.TurnedIn and is reachable from both Start and Returned, and the only unregistered flag names are the four synthesized Mission.<id>.Arrived. The owner's own frame shows the tracker had advanced to SPEAK TO THE KESS. QUESTION FOR THE OWNER, delete this in the commit that lands its answer: at Kess after the turn-in, was the "What would heat the Forge?" choice absent, present but inert, or was the tracker itself never followed back to the Anchor? Whichever it is names the runtime seam; the data does not.
...
- WalkSpeed moved 595 -> 640 alone this pass; SprintSpeed stayed at 990, narrowing the sprint gain to 1.55x. If the next report is "sprint no longer feels like anything", raise that one dial and nothing else.
