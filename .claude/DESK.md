...
- [ ] Finish Support Sympathetic's damage-independent buildup and slower allied buildup decay, distinct Entropy damage feedback, VFX/audio and ability-versus-weapon encounter tuning. Broader family encounter composition remains open.
...
- Support's Sympathetic needs a damage-independent buildup primitive and slower decay for allied applications; Attunement now delivers Entropy conversion through actual maintained buffs. Tank Kinetic Recovery consumes actual owned blast landings and protects against real fall harm and stagger.
...
- **Support Attunement:** Purchased ranks convert actual Cadence/Metronome recipients' Primary and Secondary shots to Entropy without adding hit damage. Independent source leases follow owned-window extensions, closures, expiry, death, cancellation and respec; rank two lingers for editable attunementTailSeconds (2, O2). Rockets retain fire-time conversion after cancellation. Review corrected stale conversion between window mutation and membership refresh. Build/census clean; 661 tests, 658 passed, three known failures, zero unexpected. Runtime fixture buys Section2 and Attunement within eight earned-campaign Doctrine, resets only the tested ability's cooldown between independent cases, and waits actual weapon swap/fire timing; no assertions weakened. Native Rift loop PASS24kills/2015XP/213Glass with retained return rewards, Saved/LoopProbe/1c94cafc8ca94ee49ecf67825f922d88/loop.log. Entropy is the only current element choice; Sympathetic and elemental VFX/audio remain unfinished. STATE unchanged under host-Python quarantine.
- **Entropy delivery and readouts:** Vestige melee carries an editable Entropy share; family resistance reduces buildup without reducing hit damage. The player now owns the previously missing native status component. Fracture's third cycle position earns Entropy through actual projectile impacts, including purchased multi-position casts. Enemy/player buildup rails and active Rot countdowns are inspected at 1920x1080 and 1280x720; effects expire normally. Build/census clean; 660 tests, 657 passed, three known failures, zero unexpected. Real level-10 Vestige runtime test asserts the immediately crossed threshold and exact Rot budget without changing damage values. Native Rift loop PASS24kills/2015XP/201Glass with return and reward persistence, Saved/LoopProbe/61eee8c02d0f46728c50597953347aea/loop.log. Captures use isolated visual fixtures, not a human combat-feel validation. Support element nodes, distinct damage/VFX/audio feedback and encounter balance remain unfinished. STATE unchanged under host-Python quarantine.
- **Entropy mechanical slice:** Actual accepted hits build a chassis-health threshold; earned Rot snapshots one immutable damage budget, refuses direct/duplicate inflation, expires and clears on death. Primary conversion is rollable, formatted as percent, and carried by real hitscan/rocket fire-time requests. Resistance affects buildup only. Caster Rot deals Entropy hits; legacy Void effect/factory and three obsolete Poison tuning fields are removed. Sequence uses earned Rot; Fracture retains two physical entries until element-aware delivery. Build/census pass (140 ability numbers, 65 ordinary affixes); 659 tests, 656 passed, three known failures, zero unexpected. Native Rift loop PASS24kills/2015XP/211Glass with retained rewards. Existing tests migrated for changed rules and correct zone membership without loosening assertions. New element feedback, family attacks, conversion-node support and encounter balance remain next; no visual polish claim. STATE unchanged under host-Python quarantine.
