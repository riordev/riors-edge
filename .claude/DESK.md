...
- (elements) O222 names Erased but does not define its actual effect. Proposed for review: a brief mark that pays snapshotted damage once after a delay; Tear can consume only the unpaid budget. Owner answer pending; no Erased behavior implemented from this proposal.
...
- **Safe Skirmisher anchors:** Candidate cover anchors retain full-height/distance preference but now require capsule-inset field bounds, walkable floor and blocking clearance. The existing rear offset is retained; invalid candidates yield to another real anchor or a validated open-ground fallback, never an automatic displacement through geometry. Actual wave2 checks across entry/Substation/Breach require every authored Skirmisher and verify floor/clearance/bounds. Build passed after correcting a test initializer type;678declared/started,675passed,3known,0unexpected via quarantine auditor. Gym tour frames inspected; framing/streaming limits prevent a Skirmisher visual-quality claim. Fixed ability parity comments now describe multiplier measurement rather than runtime DPS; executable model and pins unchanged. STATE unchanged under host-Python quarantine.
- **Melee cover, Parry and Rift placement:** Ordinary enemy melee checks actual3D reach and blocking geometry before damage or Entropy, and supplies the source position for purchased frontal Parry. Native regressions cover wall/open/floor separation and front/rear Parry. Fixed Rift formations reserve shared actual spawn offsets plus capsule and promotion scale, rather than900cm despite larger rings; actual entry and edge-facing wave2 bodies fit the field. Cover-selected Skirmisher placement remains separate. Build passed;677declared/started,674passed,3known,0unexpected via quarantine auditor. Native Anchor/Fernhall/Rift/reward/return probe passes24kills,2015XP,213Riftglass with persistence; accelerated kills prove lifecycle, not balance. Final first-person1080p frames inspected. STATE unchanged under host-Python quarantine.
- **Entry formation and projectile avoidance:** The first entry Rift has4melee/1Lattice, spending7/15budget with8reported unused; later waves and other encounters retain their solver. Native spawn and guaranteed starter-kit coverage pass. Dodged/parried projectile impacts no longer apply carried Bleed/Poison; accepted and status-only projectiles retain their paths. Actual traveling Fracture proves purchased Sympathetic snapshot after buff cancellation and world-timed fade. First-person1080p captures show the smaller formation; overhead tour framing was also inspected but cannot show combat clarity. Build passed;675declared/started,672passed,3known,0unexpected via quarantine auditor. STATE unchanged under host-Python quarantine. Broader Entropy balance remains open.
