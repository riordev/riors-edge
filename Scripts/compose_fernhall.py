...
# so every facade() is a stack of copies on one material and the GLB carries
# one image for all of them. The backstop box is a COPY of the pavement that
# wears the same material, its UVs pinned to the ground tile's own texel, so
# the box's bare faces read as the tile's metal; the yard's slabs keep the
# Kenney material so the builder still recognises them as ground (O279).
PIECES["panel"], PIECES["facade_window"], PIECES["facade_flat"] = load_group([
    "modular-sci-fi-megakit/glTF/Walls/WallAstra_Straight.gltf",
    "modular-sci-fi-megakit/glTF/Walls/WallAstra_Straight_Window.gltf",
    "modular-sci-fi-megakit/glTF/Walls/WallAstra_Straight_Flat.gltf"])
FACADE_MATERIAL = PIECES["panel"].visual.material
PIECES["facade_box"] = PIECES["pavement"].copy()
PIECES["facade_box"].visual = trimesh.visual.TextureVisuals(
    uv=np.tile(PIECES["panel"].visual.uv.mean(axis=0), (len(PIECES["facade_box"].vertices), 1)),
...
def facade(name, at, size, facing, storeys, columns, ends=False, skip=0):
    ...
    for f_axis, f_sign, cols in faces:
        ...
        if f_sign > 0:
            ...
            lo[f_axis] += RELIEF
    box = bake(PIECES["facade_box"].copy(), ((lo[0] + hi[0]) * 0.5, lo[1], (lo[2] + hi[2]) * 0.5), hi - lo)
    assert name not in SCENE, name
    SCENE[name] = concat_shared([box] + tiles, FACADE_MATERIAL)
