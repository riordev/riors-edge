...
try {
 $hostArgs=@(('"'+$project+'"'),'/Game/Breaker/Maps/Lvl_Fernhall?listen?BreakerCoopCombat=1','-game','-windowed','-ResX=960','-ResY=540',('-port='+$Port),'-BreakerCoopCombatTest','-nosplash','-unattended',('-abslog="'+$hostLog+'"'))
 ...
 $guestArgs=@(('"'+$project+'"'),("127.0.0.1:$Port"+'?BreakerCoopCombat=1'),'-game','-windowed','-ResX=960','-ResY=540','-BreakerCoopCombatTest','-nosplash','-unattended',('-abslog="'+$guestLog+'"'))
 ...
 } until(($ids.Count -ge 2 -and $guestText -match '\[CoopTest\].*(authority=0|received server profile)' -and $guestText -match '\[CoopTest\] local environment ready:') -or [DateTime]::UtcNow -ge $deadline)
 ...
 [pscustomobject]@{ServerProfiles=$ids;HostProcessId=$hostProcess.Id;GuestProcessId=$guestProcess.Id;Logs=$logDirectory;HostUserDir=$hostUserDirectory;GuestUserDir=$guestUserDirectory;Scope='Connection/identity smoke only; combat requires manual checks'}|ConvertTo-Json
}
finally {
