...
Purchased Conductor Attunement converts Cadence and Metronome recipients' Primary and Secondary weapon damage fully to Entropy. It adds no hit damage and combines with gear conversion by taking the higher fraction. Projectiles retain the conversion from firing. Rank two lingers after ordinary buff expiry or leaving the aura for `attunementTailSeconds` (2 seconds, O2 placeholder). Cancellation, source/recipient death and losing the node remove conversion; independent sources cannot erase each other. Entropy is the available element while Void and Rift await their own implementation.

Sympathetic Resonance adds `sympatheticFlatBuildup` (2) per eligible Entropy hit while an Attunement buff is actively maintained. This amount does not scale with hit damage; proc coefficient and resistance apply once. It helps reach Rot without enlarging Rot's snapshotted damage. Each attacker's protected buildup gets its own four-second grace, then drains linearly over `sympatheticFadeSeconds` (4); ordinary buildup still clears after four seconds. Both new values are O2 placeholders. Existing contributions survive the source buff ending and fade normally, but Attunement's linger grants no new Sympathetic payload. Weapon fire and Entropy Fracture/Rot casts snapshot the effect, so already-fired projectiles and cast zones retain it after cancellation.
