...
2. **Mark verb-granting nodes distinctly.** **Amended by O25 — Parry (Bulwark) is now the only tree-granted verb (was: "air jump and parry... the only two," LOCKED).** Air jump is base kit for everyone; Swift's third jump, if built, is a class-innate unlock, not a Core-tree grant. Parry gets a `✦` marker at the constellation level and a full-width highlighted card at the node level. Everything else in every tree is a modification of something the player already has; Parry is the one new capability and must not be mistaken for a percentage.
