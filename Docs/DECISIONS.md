...
**O271** — Rot resolves the reticle at cast completion, placing its zone under the aimed enemy or at the aimed ground; its wind-up previews the same footprint. An ordinary cast pays on the press; a queued cast pays when its own wind-up starts. A Rot recast spawns a new puddle wherever it lands; nothing merges into a live one. A press during a wind-up or Cleave recovery queues one cast, which starts when that ability finishes its recovery. Cleave wind-up and recovery both divide by the composed tree cast rate. A refused or interrupted cast makes no sound.
