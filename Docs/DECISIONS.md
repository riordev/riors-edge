...
**O176** — Swift's starters are Slipcut and Skim, the design's pair; Lead, Cadence Break, Hard Stop and Sightline are the four quartermaster unlockables. Executes as two lines in the class-definition buckets (LEDGER: BreakerProgressionLibrary.cpp starters/unlockables) landing together with KIT's DefaultAbilityIdForSlot flip; until that landing the three catalogue tests it moves are enumerated red. The token schedule needs no edit — the clamp untruncates itself at four. The v4→v5 save snapshot stays frozen at its history.
