...
**O203** — Rank on the body: an Elite wears a 2 px system-colour ellipse at its feet, a Champion two diamonds off its bar ends, and modifiers are drawn system-colour geometry above the bar, never coloured and never a fill; the coloured modifier disc and its light are retired.

**O204** — The trees keep two counters, Core and Doctrine (O111 pools), and doctrine depth stays gated per tier on the benchmark pool; adjacency on the doctrine board is drawn as links between spent nodes and grants no reachability.
