...
This is not blocking anything today: one yard exists and the interior runs in
it. It becomes load-bearing the moment yard sizes are ruled, which is why it is
here rather than in a session report.

- **Question:** when the yard shape is ruled, rule it against a spacing figure
  rather than before one. I do not need the figure now — I need the two not to
  be decided in different rooms.

## Spawn containment is LANDED, and my prediction to you was wrong

Authored, per Part One-Q's order. A pack now belongs to the field rather than
to the player's facing: the heading rotates only as far as containment
requires, the distance is the furthest the yard affords up to the authored
band, and the pack's own radius is held inside — which is the difference
between "the spawn point is in the yard" and "the pack is in the yard".

**I TOLD YOU THE 100x50 YARD WAS ALREADY THE TOO-SMALL CASE. IT IS NOT.** I
wrote the sweep expecting starvation and the assertion went red. The prediction
was true only while direction was FORCED to the player's facing, which is
precisely the defect containment removes: once a heading may rotate, a 50 m
width affords the 15 m floor from every position and every facing in the yard.
**The yard was never too small. The spawner was too rigid.**

So the question I asked you — confirm option 1, the yard declaring itself too
small — is moot at this band and this yard, and I would not build it now. It
becomes real again only if a band floor rises or a smaller yard is authored,
and `Zone.Fernhall.SpawnContainment` is pinned the strong way round to catch
exactly that: it asserts the shipped yard NEVER starves, so it goes red the day
one does. That is the build-time signal the report asked for, arriving from a
passing test rather than from a design argument.

Two things I did NOT decide, because they are still yours:

- **Whether "too small to host a fight" belongs in `IsZoneLegal`.** Unchanged
  and unasked-for now: nothing starves, so the rule would assert a condition no
  yard is in. I still lean to it being a separate query rather than a legality
  rule — the entry plaza is a real place that should never host a wave — but
  there is no longer any pressure to settle it.
- **The pack radius.** `WaveSpawnPackRadiusCm` is 900 and O2 PLACEHOLDER: above
  the widest body plus the ring a composition spreads over, chosen so the whole
  pack clears the wall rather than measured against a real spread.
