...
struct RIORSEDGE_API FBreakerWaveBudgetParams
{
    ...
    // The pause between a STANDARD wave clearing and the next arriving, in
    // every mode (GetAutoAdvanceDelay). The HUD counts it down; F4 skips it.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Waves|Cadence", meta=(ClampMin="0")) float WaveClearBreatherSeconds = 8.0f;   // O2 PLACEHOLDER
...
public:
    ...
    // clear. True in every mode: waves carry themselves, and F4 is a skip,
    // not a requirement. False after the boss wave, because the run ends
    // through O168's terminator seam and not through a wave that follows the
    // boss. The breather after a rest wave is the rest breather; after a
    // standard wave it is WaveClearBreatherSeconds.
    UFUNCTION(BlueprintPure, Category="Waves")
