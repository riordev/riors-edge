...
public:
    ...
    // counterplay is to circle to the exposed side/back, which only exists if
    // the Warden cannot re-face in a single tick. The cap that makes the flank
    // a real contest is ABreakerEnemy::MaxTurnRateDegreesPerSecond, applied in
    // the base Tick to every archetype's facing; its seed arithmetic is derived
    // against this class's SweepRangeCm and lives beside it there.
    // TickEngagedBehaviour below asks for the raw direction to the player and
    // the base turns toward it at the cap.
