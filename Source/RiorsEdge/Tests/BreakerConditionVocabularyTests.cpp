...
bool FBreakerConditionVocabularyStatTargetTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("stat targets with an aggregation lane today"), Wired, 86);
    TestTrue(TEXT("Zone/window duration has an aggregation lane"), BreakerStatTargetHasAggregationLane(EBreakerNodeStatTarget::ZoneAndWindowDuration));
    TestTrue(TEXT("Buff/window duration has an aggregation lane"), BreakerStatTargetHasAggregationLane(EBreakerNodeStatTarget::BuffAndWindowDuration));
