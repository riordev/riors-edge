...
bool FBreakerFrenzyBranchTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("Frenzy is a Swift class-point branch"), Frenzy->Currency, EBreakerPointCurrency::CorePoints);
    ...
    for (const UBreakerProgressionNode* Node : Frenzy->Nodes)
    {
        TestEqual(*(Node->NodeId.ToString() + TEXT(" reached its maximum rank")),
            Progression->GetNodeRank(Node->NodeId, EBreakerPointCurrency::CorePoints), Node->MaxRank);
    }
