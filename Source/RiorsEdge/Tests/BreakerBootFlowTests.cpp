...
{
    // O111: ONE CURRENCY. 1 Core Point per level to 50, with the slice lump
    // (12) as an ADVANCE on that entitlement rather than a bonus beside it.
    // The Class Point ladder is gone and the pool it filled is asserted at
    // ZERO below -- that assertion is the thing that stops it coming back, and
    // it is why these rows were rewritten rather than deleted. Run at the
    // shipped grants, never at an inflated test budget: granting 30 points in
    // a rig is exactly how the keystone budget contradiction stayed invisible.
    ...
    // min(Level, 50) and 12 is already advanced, so the 13th level pays the
    // 13th point. This used to be the level-11 keystone beat, which was
    // arithmetic on a Class Point budget that no longer exists.
    ...
    // THE RULING, ASSERTED. No level, no lump and no settle-up may ever put a
    // point back in the retired pool.
