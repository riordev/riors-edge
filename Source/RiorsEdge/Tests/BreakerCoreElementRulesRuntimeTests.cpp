...
bool FBreakerCoreVoidRulesRuntimeTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("Patience coefficient is the plain seventy percent; the point buys only its node"), Active()->UnpaidDamageBudget, 70.0f, .001f);
    ...
    T.Status->AdvanceStatuses(2); TestEqual(TEXT("Patience does not pay baseline expiry"), T.Health->GetHealth(), 900.0f, .001f);
    if (!TestTrue(TEXT("actual respec during delay"), F.Respec())) return false;
    T.Status->AdvanceStatuses(1); TestEqual(TEXT("snapshotted Patience pays after respec"), T.Health->GetHealth(), 830.0f, .001f);
    ...
    TestEqual(TEXT("applying hit is never added as Debt"), Active()->UnpaidDamageBudget, 70.0f, .001f);
    ...
    TestEqual(TEXT("Debt uses accepted total only once"), Active()->UnpaidDamageBudget, 85.0f, .001f);
    ...
    TestEqual(TEXT("other status damage earns finite Debt"), Active()->UnpaidDamageBudget, 86.5f, .001f);
    ...
    TestEqual(TEXT("existing delay retains Debt snapshot"), Active()->UnpaidDamageBudget, 89.5f, .001f);
    ...
    TestEqual(TEXT("Tear transfers funded original plus accepted Debt including trigger"), Reaction->Result.RawDamage, 89.65f, .001f);
    ...
    TestEqual(TEXT("expiry plus nested hit contains no refunded Debt"), T.Health->GetHealth(), 810.0f, .001f);
    ...
    TestEqual(TEXT("avoided damage adds no Debt"), Active()->UnpaidDamageBudget, 70.0f, .001f); T.Combat->DodgeChance = 0;
    Other.BaseDamage = 10000; T.Combat->ReceiveDamage(Other);
    TestEqual(TEXT("lethal overkill cannot mint debt before death cleanup"), Observer->ConsumedBudget, 70.0f, .001f);
    return true;
}
...
bool FBreakerCoreRiftRulesRuntimeTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("Hard Landing pays quarter of original burst exactly once after snapshot respec"), Wall.Health->GetHealth(), 837.5f, .001f);
    ...
    TestEqual(TEXT("support ends before the wall so Hard Landing pays no bonus"), Ledge.Health->GetHealth(), 850.0f, .001f);
    ...
    TestEqual(TEXT("blocking enemy receives one original burst"), Second.Health->GetHealth(), 950.0f, .001f);
    TestEqual(TEXT("further enemy receives no chain"), Third.Health->GetHealth(), 1000.0f, .001f);
    TestEqual(TEXT("pawn obstruction is not Hard Landing wall"), First.Health->GetHealth(), 850.0f, .001f);
    ...
    TestEqual(TEXT("equal-distance Impact uses stable actor identity"), (bAFirst ? TieA : TieB).Health->GetHealth(), 950.0f, .001f);
    ...
    TestEqual(TEXT("immunity still pays ordinary earned burst"), Immune.Health->GetHealth(), 850.0f, .001f);
