...
public:
    ...
    // out an Unwritten affix count, then its rule.
    //
    // A legendary is always Unwritten, which means the existing equip cap of
    // ONE Unwritten piece is also the cap on legendaries. That is the design,
    ...
    // O2 PLACEHOLDER. Chance that an Unwritten drop in a slot that HAS a
    // legendary is that legendary instead of an ordinary rule roll. Unwritten
    ...
    static constexpr float LegendaryChanceWithinUnwritten = 0.25f;
